...
| `secret.pmm_password` | Initial PMM password - it changes only on the first deployment, ignored if PMM was already provisioned and just restarted. If PMM admin password is not set, it will be generated. Only used when secret.create is true; with a manually created secret the PMM_ADMIN_PASSWORD key is required instead. | `""`         |
...
is ignored and the secret's `PMM_ADMIN_PASSWORD` key is required: the chart fails the install if
it is missing, because without it PMM would silently keep its built-in admin password.
...
> install if a pre-created secret does not contain it. `GF_PASSWORD` is unrelated to the UI
> login - it is the password of the Grafana **PostgreSQL** database user.
